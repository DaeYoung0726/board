package com.Board.project_board.config.handler;
...
@Component
public class CustomAuthFailureHandler extends SimpleUrlAuthenticationFailureHandler {   // 로그인 에러 예외 코드.
    @Override
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
                                        AuthenticationException exception) throws IOException, ServletException {
        String errorMessage = getErrorMessage(exception);

        errorMessage = URLEncoder.encode(errorMessage, StandardCharsets.UTF_8);     // 한글로 나올 수 있게 하기 위해.
